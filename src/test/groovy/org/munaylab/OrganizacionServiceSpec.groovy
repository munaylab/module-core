...
class OrganizacionServiceSpec extends SpecificationTestBuilder
    ...
    void 'validacion de parametros de registro'() {
        expect:
        service.siLosParametrosSonInvalidos(command) == result
        where:
        command                     | result
        commandDeRegistroValido     | false
        commandDeRegistroInvalido   | true
    }

    void 'obtener usuario de parametros de registro'() {
        when:
        def user = service.obtenerRepresentanteConDatosDeContacto(commandDeRegistroValido)
        then:
        comprobarQueLosDatosDelUsuarioSonLosMismosQueDelCommand(user, commandDeRegistroValido)
    }

    void 'obtener organizacion de parametros de registro'() {
        when:
        def org = service.obtenerOrganizacion(commandDeRegistroValido)
        then:
        comprobarQueLosDatosDeLaOrganizacionSonLosMismosQueDelCommand(org, commandDeRegistroValido)
    }

    void 'asignar administrador de organizacion'() {
        given:
        def user = usuarioValido
        def org = organizacionValida
        tipoDeUsuarioAdministrador.save()
        when:
        org = service.asignarAdminDeOrganizacion(user, org)
        then:
        comprobarQueLaOrganizacionTieneAsignadoUnAdministrador(org, user)
    }

    void 'error al asignar administrador de organizacion'() {
        when:
        service.asignarAdminDeOrganizacion(null, null)
        then:
        thrown Exception
    }

    void 'asignar rol de administrador de organizacion'() {
        given:
        rolDeAdministradorDeOSC.save()
        when:
        def userRol = service.asignarRolDeAdminDeOrganizacion(usuarioValido)
        then:
        comprobarQueLosDatosDelRolSonCorrectos(userRol, usuarioValido)
    }

    void 'error al asignar el rod de administrador de organizacion'() {
        when:
        service.asignarRolDeAdminDeOrganizacion(null)
        then:
        thrown Exception
    }

    void 'registrar organizacion valida'() {
        given:
        rolDeAdministradorDeOSC.save()
        tipoDeUsuarioAdministrador.save()
        when:
        def org = service.registrar(commandDeRegistroValido)
        then:
        comprobarQueElRegistroSeRealizoCorrectamente(org, commandDeRegistroValido)
    }

    void 'registrar organizacion con parametros invalidos'() {
        expect:
        service.registrar(commandDeRegistroInvalido) == null
    }

    void 'registrar organizacion con errores'() {
        when:
        service.registrar(commandDeRegistroValido)
        then:
        thrown Exception
    }
